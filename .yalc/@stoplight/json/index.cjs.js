...
var createOrderedObject = require('@stoplight/ordered-object-literal');
var createOrderedObject__default = _interopDefault(createOrderedObject);
var types = require('@stoplight/types');
var fastStringify = _interopDefault(require('safe-stable-stringify'));

const hasRef = (obj) => lodash.isObject(obj) && '$ref' in obj && typeof obj.$ref === 'string';
...
const bundleTarget = ({ document, path }, cur) => _bundle(lodash.cloneDeep(document), path, { [path]: true }, cur);
const _bundle = (document, path, stack, cur, bundledRefInventory = {}, bundledObj = {}, errorsObj = {}) => {
    const objectToBundle = lodash.get(document, pointerToPath(path));
    traverse(cur ? cur : objectToBundle, ({ parent }) => {
        if (hasRef(parent) && isLocalRef(parent.$ref)) {
            ...
            if (!_path || !inventoryPath || !inventoryRef)
                return;
            const bundled$Ref = lodash.get(document, _path);
            if (bundled$Ref) {
                const pathProcessed = [];
                ...
                for (const key of _path) {
                    ...
                    if (lodash.has(bundledObj, inventoryPathProcessed))
                        continue;
                    const target = lodash.get(document, pathProcessed);
                    if (Array.isArray(target)) {
                        lodash.set(bundledObj, inventoryPathProcessed, new Array(target.length).fill(null));
                    }
                    else if (typeof target === 'object') {
                        lodash.set(bundledObj, inventoryPathProcessed, {});
                    }
                }
                lodash.set(bundledObj, inventoryPath, bundled$Ref);
                if (!stack[$ref]) {
                    stack[$ref] = true;
    ...
    lodash.set(objectToBundle, BUNDLE_ROOT, bundledObj[BUNDLE_ROOT]);
    if (Object.keys(errorsObj).length) {
        lodash.set(objectToBundle, ERRORS_ROOT, errorsObj);
    }
...
const getFirstPrimitiveProperty = (text) => {
    const scanner = jsoncParser.createScanner(text, true);
    scanner.scan();
...
const getJsonPathForPosition = ({ lineMap, ast }, position) => {
    ...
    const node = jsoncParser.findNodeAtOffset(ast, endOffset === void 0 ? startOffset + position.character : Math.min(endOffset, startOffset + position.character), true);
    if (node === undefined) {
        return;
    }
    const path = jsoncParser.getNodePath(node);
    if (path.length === 0)
        return;
    return path;
};
...
function parseTree(text, errors = [], options) {
    ...
    const visitor = {
        ...
        onError: (error, offset, length, startLine, startCharacter) => {
            errors.push({
                range: calculateRange(startLine, startCharacter, length),
                message: jsoncParser.printParseErrorCode(error),
                severity: types.DiagnosticSeverity.Error,
                code: error,
            });
        },
    };
    jsoncParser.visit(text, visitor, options);
...
function createObjectLiteral(preserveKeyOrder) {
    return preserveKeyOrder ? createOrderedObject__default({}) : {};
}
function swapKey(container, key) {
    if (!(key in container))
        return;
    const order = createOrderedObject.getOrder(container);
...
const KEYS = Symbol.for(createOrderedObject.ORDER_KEY_ID);
const traps$1 = {
    ownKeys(target) {
...
function trimStart(target, elems) {
    if (typeof target === 'string' && typeof elems === 'string') {
        return lodash.trimStart(target, elems);
    }
...
exports.ERRORS_ROOT = ERRORS_ROOT;
exports.KEYS = KEYS;
...
exports.encodePointerFragment = encodePointerFragment;
exports.extractPointerFromRef = extractPointerFromRef;
exports.extractSourceFromRef = extractSourceFromRef;
exports.getFirstPrimitiveProperty = getFirstPrimitiveProperty;
...
exports.lazyInlineResolver = lazyInlineResolver;
...
exports.resolveInlineRef = resolveInlineRef;
exports.safeParse = safeParse;
exports.safeStringify = safeStringify;
exports.startsWith = startsWith;
exports.stringify = stringify;
exports.toPropertyPath = toPropertyPath;
exports.trapAccess = trapAccess;
